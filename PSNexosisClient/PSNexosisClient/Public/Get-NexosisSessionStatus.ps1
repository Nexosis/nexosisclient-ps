Function Get-NexosisSessionStatus {
<# 
 .Synopsis
 ...
 .Example
  ...
  Get-NexosisSessionStatus -SessionID 015da45b-2ee7-4a63-b6c7-2f3798ea70a2

 .Example
 # For each retrieved session for dataset named 'salesdata', get the current status
 (Get-NexosisSession -dataSetName salesdata) | foreach { "SessionId: " + $_.SessionID + " Status: " + (Get-NexosisSessionStatus $_.SessionId) }
#>[CmdletBinding()]
	Param(
