...
Describe "Get-Import" {
	Context "unit tests" {
		...
		It "gets imports by dataset name and requested before and after dates including paging defaults" {
            Get-Import -dataSetName 'salesdata' -requestedAfterDate 2017-01-01T00:00:00Z -requestedBeforeDate 2017-01-20T00:00:00Z 
            Assert-MockCalled Invoke-WebRequest -ModuleName PSNexosisClient -Times 1 -Scope It -ParameterFilter {
				$Uri -eq "$($TestVars.ApiEndPoint)/imports?dataSetName=salesdata&requestedAfterDate=12%2f31%2f2016+19%3a00%3a00&requestedBeforeDate=01%2f19%2f2017+19%3a00%3a00"
			} 
		}
		...
		It "should throw error with invalid pagesize" {
			{Get-Import -dataSetName 'salesdata' -page 1 -pageSize 1001} | should throw "Parameter '-pageSize' must be an integer between 0 and $($TestVars.MaxPageSize)."
		}

		It "should throw error with invalid page param" {
			{Get-Import -dataSetName 'salesdata' -page 1001 -pageSize 100} | should throw "Parameter '-page' must be an integer between 0 and $($TestVars.MaxPageSize)."
		}
