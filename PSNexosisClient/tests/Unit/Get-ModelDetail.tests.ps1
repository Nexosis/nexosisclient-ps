...
Describe "Get-NexosisModelDetail" -Tag 'Unit' {
	Context "unit tests" {
        ...
        It "gets model detail by ModelId" {
           
            Get-NexosisModelDetail -modelId $modelId
		...
		It "throws exception when modelId is invalid" {
			{Get-NexosisModelDetail -modelId ''} | Should throw "Cannot process argument transformation on parameter 'ModelId'. Cannot convert value `"`" to type `"System.Guid`". Error: `"Unrecognized Guid format.`""
		}
