...
Describe "Get-NexosisVocabulary" -Tag 'Unit' {
	Context "unit tests" {
        ...
        It "returns a session result" {
            Get-NexosisVocabularySummary -dataSource source -createdFromSession $sessionId -page 1 -pageSize 20
        ...
        It "calls with the proper URI" {
			Assert-MockCalled Invoke-WebRequest -ModuleName PSNexosisClient -Times 1 -Scope Context -ParameterFilter {
				$Uri -eq "$($TestVars.ApiEndPoint)/vocabulary?createdFromSession=$($sessionId)&datasource=source&page=1&pageSize=20"
			} 
		}
