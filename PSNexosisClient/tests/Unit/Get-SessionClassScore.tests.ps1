...
Describe "Get-NexosisSessionClassScore" -Tag 'Unit' {
	Context "unit tests" {
        ...
        It "returns a session result" {
            Get-NexosisSessionClassScore -sessionId $sessionId
        ...
        It "calls with the proper URI" {
			Assert-MockCalled Invoke-WebRequest -ModuleName PSNexosisClient -Times 1 -Scope Context -ParameterFilter {
				$Uri -eq "$($TestVars.ApiEndPoint)/sessions/$sessionId/results/classscores"
			} 
		}
		...
		It "loads class score with paging" {
			$results = Get-NexosisSessionClassScore -sessionId $sessionId -page 1 -pageSize 100 
			Assert-MockCalled Invoke-WebRequest -ModuleName PSNexosisClient -Times 1 -Scope It -ParameterFilter {
				$Uri -eq "$($TestVars.ApiEndPoint)/sessions/$sessionId/results/classscores?page=1&pageSize=100"
			} 
		}
    }
}